...
**Admin is the exception, not a violation of this pattern.** There is no `docs/api/admin.md` (see open question 1), so step 1 has nothing to read — Admin screens (Users, Roles, Products, Depots) stay on the older, simpler chain from "Layer architecture" above: `types/` → `mock/` → `services/` (no Dto, no mock/real switch, no `Paged<T>`) → `hooks/` → `pages/`. `productService.ts` and `depotLocationService.ts` are real, working CRUD services in this simpler sense — not a "temporary stand-in" like `catalogService.ts`, which delegates its `getProducts`/`getDepots` to them so Factory/Depot form selectors stay in sync with Admin edits. Do not invent an Admin API contract to force-fit the real-backend pattern here; wait for the doc.
