...
function CreateSupplyForm() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<SupplyCreateValues>({
    resolver: zodResolver(supplyCreateSchema) as Resolver<SupplyCreateValues>,
  });
  ...
  async function onSubmit(values: SupplyCreateValues) {
    ...
    try {
      ...
      navigate(ADMIN_PATHS.factory.supplies.list);
    } catch (err) {
      setSubmitError(getApiErrorMessage(err, 'Failed to create supply record.'));
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mt-6 bg-white border border-gray-200 rounded-lg p-6 flex flex-col gap-5">
      ...
      <FormField label="Quantity ID" error={errors.quantityId?.message} required>
        <input {...register('quantityId')} type="number" min={1} className={inputClasses} placeholder="e.g. 1" />
      </FormField>
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-gray-100">
        ...
        <button
          type="button"
          onClick={() => navigate(ADMIN_PATHS.factory.supplies.list)}
...
function DecideSupplyForm({ record }: { record: SupplyRecord }) {
  ...
  const {
    ...
    resolver: zodResolver(supplyDecideSchema),
    defaultValues: { status: record.status === 'pending' ? 'received' : record.status, rejectionReason: record.rejectionReason ?? '' },
  });
  ...
  async function onSubmit(values: SupplyDecideValues) {
    ...
    try {
      ...
      setSubmitError(getApiErrorMessage(err, 'Failed to update supply record.'));
    } finally {
      setIsSubmitting(false);
    }
  }
