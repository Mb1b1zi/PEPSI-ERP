...
import type { ProductProductionBreakdown } from '@/hooks/useProductionReport';
...
const columns: Column<ProductProductionBreakdown>[] = [
  { header: 'Product', render: (p) => <span className="font-medium text-gray-900">{p.productName}</span> },
  { header: 'Units Produced', render: (p) => p.totalProduced.toLocaleString() },
  { header: 'Production Runs', render: (p) => p.recordCount },
];
...
export function ProductionReportPage() {
  ...
  const report = useProductionReport({ dateFrom: range.dateFrom, dateTo: range.dateTo });

  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold text-gray-900">Production Report</h1>
      <p className="text-gray-500 mt-1">Factory-wide output, broken down by product.</p>
      ...
      {(period === 'week' || period === 'month' || period === 'custom') && (
        <div className="mt-6 bg-blue-50 border border-blue-200 text-blue-700 text-sm rounded-md px-4 py-3">
          Multi-day production reports load one request per day (the Factory API only accepts a single
          exact date, not a range) — this may take a moment for longer ranges.
        </div>
      )}
      ...
      {report.isTruncated && !report.isLoading && (
        <div className="mt-6 bg-amber-50 border border-amber-200 text-amber-700 text-sm rounded-md px-4 py-3">
          This range has more production records than could be fully summed — totals below reflect only a
          partial set. Narrow the date range for an exact total.
        </div>
      )}
      ...
      {report.isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
          <StatCard label="Units Produced" value={report.totalProduced.toLocaleString()} icon={<Boxes size={20} />} />
          <StatCard label="Production Runs" value={report.recordCount} icon={<ClipboardList size={20} />} />
        </div>
      )}
      ...
      <div className="mt-6">
        {report.isLoading ? (
          ...
          <Table
            ...
            emptyMessage="No production recorded in this range."
          />
        )}
      </div>
    </div>
